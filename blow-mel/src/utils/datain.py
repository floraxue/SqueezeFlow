...
class DataSet(torch.utils.data.Dataset):
    ...
    def __init__(self, path_in, lchunk, stride, split='train', trim=None,
                 ...
                 frame_energy_thres=0.02, temp_jitter=False,
        ...
        self.stride = stride
        self.split = split
        self.frame_energy_thres = frame_energy_thres
        ...
        if do_audio_load:
            for i, filename in enumerate(self.filenames):
                ...
                for j in range(0, len(x), stride):
                    ...
                    if (xx.pow(
                            2).sum() / self.lchunk).sqrt().item() >= frame_energy_thres:
                        info = [i, j, 0, ispk, iut]
    ...
    def get_whole_audio(self, idx):
        ...
        x = x.float()
        filtered = []
        for j in range(0, len(x), self.lchunk):
            if j + self.lchunk >= len(x):
                xx = x[j:]
            else:
                xx = x[j:j + self.lchunk]
            if (xx.pow(2).sum() / self.lchunk).sqrt().item() >= self.frame_energy_thres:
                filtered.append(xx)
        x = torch.cat(filtered)
        # Info
        spk, ut = self.filename_split(self.filenames[idx])
        ispk, iut = self.speakers[spk], self.utterances[ut]
        info = torch.LongTensor([idx, 0, 1, ispk, iut])
        return x, info
    ...
    def __getitem__(self, idx):
        if self.split == 'test':
            return self.get_whole_audio(idx)
